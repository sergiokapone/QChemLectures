...
%============================================================================
\begin{frame}{Межі застосування молекулярної механіки}{Температура виродження}
	\begin{alertblock}{Характеристична температура коливань}\small\justifying
		\begin{equation*}
			\Theta_i = \frac{\hbar\,\omega_i}{k_B}
		\end{equation*}
		При $T \ll \Theta_i$ дана коливальна мода «заморожена» --- підпорядковується \emphs{квантовій} статистиці, класичні методи дають хибний результат.
	\end{alertblock}
	\emphs[red]{Класична ММ некоректна, якщо:}
	\begin{itemize}
		\item $T \ll \Theta_i$ --- нульова коливальна енергія $\tfrac{1}{2}\hbar\omega$ не врахована
		\item Присутні \emphs{легкі атоми} (H, D) --- тунелювання суттєве навіть при $T = 300$~К
		\item Досліджуються \emphs{збуджені електронні стани} або фотохімічні реакції
		\item Відбувається \emphs{розрив або утворення} хімічних зв'язків
		\item Параметри силового поля екстрапольовані \emphs{поза межі} навчальної вибірки
	\end{itemize}
\end{frame}
%============================================================================
...
\begin{frame}{Сили та потенціальна енергія}{}
	\begin{overprint}
		...
		\framesubtitle<3>{\href{https://avogadro.cc/docs/optimizing-geometry/molecular-mechanics/}{Силові поля, реалізовані в Avogadro}}
...
%============================================================================
\begin{frame}{Молекулярна динаміка}{Розвиток молекулярної механіки}\small
	\begin{block}{}
		\emphs[red]{Молекулярна динаміка (МД)} --- рух атомів описується \emphs{рівняннями Ньютона}:
		\begin{equation*}
			m_i\,\ddot{\vec{r}}_i = -\vec{\nabla}_i U(\vec{r}_1,\dots,\vec{r}_N),
		\end{equation*}
		де сила обчислюється з тих самих емпіричних потенціалів, що й у ММ.
	\end{block}

	\begin{block}{}\scriptsize
		\emphs[red]{МД дозволяє розраховувати:}
		\begin{itemize}
			\item пружні константи ($C_{11}$, $C_{12}$, $C_{44}$)
			\item теплопровідність (метод Гріна--Кубо)
			\item дифузію, конформаційні зміни
			\item температурну залежність властивостей
		\end{itemize}
		\vspace{0.4em}
		\emphs[red]{Типові програми:}
		\begin{itemize}
			\item \href{https://www.lammps.org/\#gsc.tab=0}{LAMMPS} --- кристали, матеріали
			\item \href{https://www.gromacs.org}{GROMACS / NAMD} --- білки, ДНК
			\item \href{https://www.ovito.org}{OVITO} --- візуалізація траєкторій
		\end{itemize}
	\end{block}
\end{frame}
%============================================================================
...
%============================================================================
\begin{frame}{Чому не квантова механіка?}\small
	\begin{alertblock}{}\centering
		Вибір методу — це компроміс між точністю та обчислювальною вартістю.
	\end{alertblock}
	\begin{center}\scriptsize
		\begin{tabular}{lccc}
			\toprule
			\textbf{Метод} & \textbf{Масштаб} & \textbf{Атомів} & \textbf{Точність} \\
			\midrule
			Ab initio (CCSD(T)) & $O(N^7)$ & $<50$        & висока \\
			DFT                 & $O(N^3)$ & $<500$       & середня \\
			Молекулярна механіка & $O(N^2)$ & $<10^6$     & емпірична \\
			Молекулярна динаміка & $O(N)$   & до $10^9$   & емпірична \\
			\bottomrule
		\end{tabular}
	\end{center}
	\begin{columns}[t]\scriptsize
		\begin{column}{0.5\linewidth}
			\emphs[red]{ММ/МД виправдана, якщо:}
			\begin{itemize}
				\item система велика ($>10^3$ атомів)
				\item хімічні зв'язки не рвуться
				\item збуджені стани не потрібні
				\item потрібна часова динаміка (нс і більше)
				\item $T \gg \Theta_i = \hbar\omega_i/k_B$ --- коливання класичні
			\end{itemize}
		\end{column}
		\begin{column}{0.5\linewidth}
			\emphs[red]{Квантова хімія необхідна, якщо:}
			\begin{itemize}
				\item відбувається хімічна реакція
				\item важливі електронні ефекти
				\item потрібні спектри, збуджені стани
				\item система мала і потрібна висока точність
				\item $T \ll \Theta_i$ --- квантові ефекти в коливаннях (легкі атоми)
			\end{itemize}
		\end{column}
	\end{columns}
	\begin{block}{}\small\centering
		Компроміс --- гібридний метод \emphs{QM/MM}: квантова хімія для активного центру молекули + ММ для оточення.
	\end{block}
\end{frame}
%============================================================================
