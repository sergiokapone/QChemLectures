...
\begin{frame}{Розкладання за мультиполями}{}\small
	...
	\begin{overprint}
		...
		\onslide<2>
		%        [title = {Заряд}]
		%        [title = {Дипольний момент}]
		%        [title = {Момент 2-го порядку}]
		...
		\begin{align*}
			\phi^{\rho}(\vec R) & = \frac{1}{|\vec R - \vec{r}_0|} \int\limits_{\vec r'} \rho(\vec r')d\vec r' \, +                                                                  \\
			                    & + \sum_\alpha\left( \frac{\partial }{\partial r'_\alpha} \frac{1}{|\vec R - \vec r'|}\right)_{\vec r' = \vec r_0}
			\int\limits_{\vec r'} \rho(\vec r')(r'_\alpha - r_{0,\alpha})d\vec r'  \, +                                                                                              \\
			                    & + \frac12\sum_{\alpha\beta}\left( \frac{\partial^2 }{\partial r'_\alpha\partial r'_\beta} \frac{1}{|\vec R - \vec r'|}\right)_{\vec r' = \vec r_0}
			\int\limits_{\vec r'} \rho(\vec r')(r'_\alpha - r_{0,\alpha})(r'_\beta - r_{0,\beta})d\vec r' \, + \ldots
		\end{align*}
	\end{overprint}
\end{frame}
%============================================================================
...
\begin{frame}{Розкладання за мультиполями}{}
	\begin{itemize}\small
		...
		\item Електричні моменти функції розподілу заряду $\rho(\vec r)$:
		      %            \begin{equation*}
		      %                \tcbhighmath[title ={\scriptsize Електричний заряд}]{
		      %                q  = \int\limits_{\vec r'}  \rho(\vec r') d\vec r'
		      %                }
		      %            \end{equation*}
		      %        \begin{equation*}
		      %            \tcbhighmath[title ={\scriptsize Дипольний момент}]{
		      %                \mu_\alpha(\vec r_0)  = \int\limits_{\vec r'}  \rho(\vec r') (r'_\alpha - r_{0,\alpha}) d\vec r'
		      %            }
		      %        \end{equation*}
		      %    \begin{equation*}
		      %        \tcbhighmath[title ={\scriptsize  Момент 2-го порядку}]{
...
\begin{frame}{Потенціальна енергія в електричному полі}{}\small
	...
	\begin{equation*}
		\tcbhighmath[title = {Енергія взаємодії зарядів з полем}]{
			E(\vec{\mathcal{E}})
	...
	\begin{equation*}
		\tcbhighmath[title = {дипольний момент}]{
			\mu_\alpha(\vec r_0) = -\frac{dE(\mathcal{E})}{d \mathcal{E}_\alpha(\vec r_0)},
		}
		\quad
		\tcbhighmath[title = {момент 2-го порядку}]{
			Q_{\alpha\beta}(\vec r_0) =  -2\frac{d E(\mathcal{E})}{d \mathcal{E}_{\alpha\beta}(\vec r_0)}
		}
	\end{equation*}
	разом з означеннями постійних (за відсутності поля) моментів
	\begin{equation*}
		\mu_\alpha^\text{perm}(\vec r_0)       = \int\limits_{\vec r'}  \rho(\vec r') (r'_\alpha - r_{0,\alpha}) d\vec r',
		\quad
		Q_{\alpha\beta}^\text{perm}(\vec r_0)  = \int\limits_{\vec r'} \rho(\vec r')(r'_\alpha - r_{0,\alpha})(r'_\beta - r_{0,\beta})d\vec r'
	\end{equation*}
	\begin{alertblock}{}\footnotesize\centering
		Індекс <<perm>> означає <<постійні>> моменти (на відміну від <<індукованих>>).
	\end{alertblock}
\end{frame}
%============================================================================
...
% ============================== Слайд #6 ===================================
\begin{frame}{Індуковані дипольні моменти}{}\small
	\begin{itemize}
	...
	\end{itemize}
		...
		\begin{equation*}\label{}
			...
			\alpha_{\alpha\beta} = \left.\frac{d\mu_\alpha}{d\mathcal{E}_\beta}\right|_{\vec{\mathcal{E}} = 0}
			}
			\quad
			\tcbhighmath[title = {\scriptsize гіперполяризовність}]{
			\beta_{\alpha\beta\gamma} = \left.\frac{d\mu_\alpha}{d\mathcal{E}_\beta d\mathcal{E}_\gamma}\right|_{\vec{\mathcal{E}} = 0}
			}
		\end{equation*}
%		\onslide<2> Розкладання квадрупольного моменту в ряд
%		\begin{equation*}\label{}
%			Q_{\alpha\beta}(\vec{\mathcal{E}}) = Q_{\alpha\beta}^\text{perm} + \sum_\gamma \Alpha_{\gamma, \alpha\beta}\mathcal{E}_\gamma + \frac12\sum_{\gamma\delta} \Beta_{\gamma\delta, \alpha\beta} \mathcal{E}_\gamma \mathcal{E}_\delta + \ldots,
%		\end{equation*}
%		Дипольно-квадрупольні поляризовність та гіперполяризовність
%		\begin{equation*}\label{}
%			\tcbhighmath[title = {\scriptsize поляризовність}]{
%			\Alpha_{\gamma, \alpha\beta} = \left.\frac{dQ_{\alpha\beta}}{d\mathcal{E}_\gamma}\right|_{\vec{\mathcal{E}} = 0}
%			}
%			\quad
%			\tcbhighmath[title = {\scriptsize гіперполяризовність}]{
%			\Beta_{\gamma\delta, \alpha\beta} = \left.\frac{dQ_{\alpha\beta}}{d\mathcal{E}_\gamma d\mathcal{E}_\delta}\right|_{\vec{\mathcal{E}} = 0}
%			}
%		\end{equation*}
...
% ============================== Слайд #7 ===================================
\begin{frame}{Індуковані дипольні моменти}{}
	\begin{itemize}
		\item Інфінітезимальна зміна енергії при інфінітезимальній зміні поля  $d\mathcal{E}_\alpha$ (дипольна складова)
		      \begin{equation*}
		...
		\item Енергію можна отримати інтегруванням
		      \begin{multline*}
			      ...
			      = -\sum_\alpha \mu_\alpha^\text{perm}\mathcal{E}_\alpha - \frac12\sum_{\alpha\beta} \alpha_{\alpha\beta\gamma}\mathcal{E}_\alpha\mathcal{E}_\beta - \frac16\sum_{\beta\gamma} \beta_{\alpha\beta\gamma} \mathcal{E}_\alpha\mathcal{E}_\beta\mathcal{E}_\gamma + \ldots
		      \end{multline*}
...
%	\begin{itemize}
%		\item Інфінітезимальна зміна енергії при інфінітезимальній зміні градієнта поля  $d\mathcal{E}_\alpha$ (квадрупольна складова)
%		      \begin{equation*}
%  			      dE = - \sum_{\alpha\beta} Q_{\alpha\beta}(\vec{\mathcal{E}}) d\mathcal{E}_{\alpha\beta}
%		      \end{equation*}
%		\item Енергію можна отримати інтегруванням
%		      \begin{multline*}
%			      E(\mathcal{E}) - E_0 = -\int\limits_0^{\mathcal{E}_{\alpha\beta}} \sum_{\alpha\beta} Q_{\alpha\beta}(\vec{\mathcal{E}})d\mathcal{E}_{\alpha\beta }  = \\
%			      = -\sum_{\alpha\beta} Q_{\alpha\beta}^\text{perm}\mathcal{E}_{\alpha\beta} - \frac12\sum_{\alpha\beta} \Alpha_{\alpha{\beta\gamma}}\mathcal{E}_\alpha\mathcal{E}_\beta + \ldots
%		      \end{multline*}
%		\item Властивості за відсутності поля ($\vec{\mathcal{E}} = 0$) можна отримати диференціюючи енергію:
%	\end{itemize}
%	\begin{equation*}
%		Q_{\alpha\beta}^\text{perm} = - \left.\frac{dE}{d\mathcal{E}_{\alpha\beta}}\right|_{\vec{\mathcal{E}}=0}
%		\,\,
%		\alpha_{\alpha\beta} = - \left.\frac{d^2E}{d\mathcal{E}_\alpha d\mathcal{E}_\beta}\right|_{\vec{\mathcal{E}}=0}
%		\,\,
%		\beta_{\alpha\beta\gamma} = - \left.\frac{d^3E}{d\mathcal{E}_\alpha d\mathcal{E}_\beta d\mathcal{E}_\gamma}\right|_{\vec{\mathcal{E}}=0}
...
\begin{frame}{Теорія відгуку}{Відгук --- реакція на збурення}
	...
	\begin{alertblock}{}\centering
		Молекулярні властивості можуть бути отримані з використанням похідних електронної енергії або молекулярних моментів за збуренням!
	\end{alertblock}
...
\begin{frame}{Магнітні властивості}{}\small
	\begin{itemize}
		\item Взаємодію з магнітним полем можна записати через магнітні дипольні, квадрупольні, \ldots{} моменти ({\scriptsize магнітного монополя немає}).
		\item  Оскільки магнітна взаємодія є значно меншою за величиною, ніж електрична, зазвичай розглядається лише дипольний член:
		      \begin{equation*}
			      m_\alpha = -\frac{dE(\vec{\mathcal{B}})}{d\mathcal{B}_\alpha}
		      \end{equation*}
          \item Індукований магнітний момент $m_\alpha(\vec{\mathcal{B}}) = m_\alpha^\text{perm} +  m_\alpha^\text{ind} $
          \begin{equation*}
              m_\alpha(\vec{\mathcal{B}}) = m_\alpha^\text{perm} +  \sum_\beta \xi_{\alpha\beta} \mathcal{B}_\beta + \sum_K \sum_\beta \sigma^K_{\alpha\beta} m_\beta^K + \ldots
          \end{equation*}
          Окрім зовнішніх полів, на електронну оболонку діє магнітне поле ядерних спінів.
%		\item Енергія взаємодії магнітного диполя з магнітним полем
%		      \begin{equation*}
%			      E(\mathcal{B}) = E_0 -\sum_\beta m_\alpha\mathcal{B}_\alpha - \frac12\sum_\beta \alpha_{\alpha\beta}\mathcal{B}_\alpha\mathcal{B}_\beta  + \ldots
%		      \end{equation*}
%		\item Властивості за відсутності поля ($\vec{\mathcal{B}} = 0$) можна отримати диференціюючи енергію:
%		      \begin{equation*}
%			      \text{\scriptsize магнітний момент}\, m_\alpha = - \left.\frac{dE}{d\mathcal{B}_\alpha}\right|_{\vec{\mathcal{B}}=0} \,\,
...
%		      \end{equation*}
		\item Намагнічованість $\xi_{\alpha\beta}$ --- характеристика молекули. ({\scriptsize Відповідна макроскопічна величина називається магнітною сприйнятливістю.})
        \item $m_\beta^K$ --- магнітний момент $K$-го ядра молекули, $\sigma^K_{\alpha\beta}$ --- ядерний магнітний тензор.
	\end{itemize}
\end{frame}
%============================================================================
...
\begin{frame}{Магнітні властивості}{}\small
	\begin{itemize}
		\item     Дипольний момент $\mu_\alpha$ для незбуреної системи залежить від повного електронного моменту імпульсу $\vec L$, та електронного спіну $\vec S$ (в атомних одиницях):
		      \begin{equation*}
			      \vec{m} =-\frac12 \left( \vec L - g_e\vec S \right)
		      \end{equation*}
