...
%% ========================================================
\section{Метод CPHF: орбітальні параметри та їхня лінійна реакція}
%% ========================================================
...
Замість цього варіацію формулюють через \emph{нескінченно малі орбітальні обертання}.
Будь-яка допустима зміна МО може бути записана у вигляді
\begin{equation}\label{eq:small_rotation_intro}
\phi_p \;\longrightarrow\;
\phi_p^{(0)} + \sum_q U_{pq}\,\phi_q^{(0)},
\end{equation}
де $\mathbf U$ --- антисиметрична матриця:
\[
U_{pq}=-U_{qp}.
\]

\paragraph{Варіаційні параметри.}
У детермінанті фізично значимими є лише ті обертання, що змішують
\emph{зайняті} та \emph{віртуальні} орбіталі.
Обертання всередині підпросторів зайнятих або віртуальних МО
не змінюють хвильову функцію (це просто зміна базису всередині
однаково заселених підпросторів). Тому саме елементи
\begin{equation}\label{eq:p_HF_definition}
p_{ai} \equiv U_{ai}
\end{equation}
є варіаційними параметрами методу \texttt{HF}.
Вектор параметрів $\mathbf p$ складається саме з них:
\[
\mathbf p = \{U_{ai}\}, \qquad
i\ \text{--- зайнята орбіталь}, \quad
a\ \text{--- віртуальна}.
\]
...
g_{ai}^{(x)}
=
\left.\frac{\partial^2 E}{\partial x\,\partial U_{ai}}\right|_{\mathbf U= 0}.
\end{equation}
...
\[
g_{ai}^{(x)} = \langle \phi_a | \hat h^{(x)} + \hat V^{(x)}_{ee} | \phi_i \rangle,
\]
...
Для кожної компоненти поля \(\mu\) визначаємо похідну фокіана по збуренню:
\[
\hat F^{(\mu)} = \frac{\partial \hat F}{\partial \mathcal E_\mu}
= \frac{\partial \hat h}{\partial \mathcal E_\mu}
= -\hat\mu_\mu,
\]
і відповідні праві частини рівнянь \eqref{eq:CPHF_explicit}:
...
Розв’язуючи, знаходять $U^{(\mu)}_{ai}$.


% ------------------------------------------------------------

Компоненти зміненої густини:
\begin{equation}
D^{(\mu)}_{ai} = U^{(\mu)}_{ai},
\qquad
D^{(\mu)}_{ia} = U^{(\mu)}_{ai},
...
Перший порядок Фока за \(\mathcal E_\nu\):
\begin{equation}
F^{(\nu)} = \frac{\partial F}{\partial \mathcal E_\nu}
= \frac{\partial h}{\partial \mathcal E_\nu}
= -\mu^{(\nu)}.
\end{equation}

Тоді змішана друга похідна енергії у методі HF дорівнює
...
Підставивши результат у означення \eqref{eq:alpha_def}, отримуємо:
...
Тобто поляризовність у методі HF визначається як подвійний
проєкційний добуток між лінійною реакцією орбіталей у полі \(\mathcal E_\mu\)
...
\begin{enumerate}
    \item Розрахувати незбурений HF та отримати МО, енергії \(\varepsilon_p\), інтеграли \((pq|rs)\), матриці \(\mu^{(\lambda)}\).
    \item Для кожної компоненти поля \(\mu = x,y,z\):
    \[
...
% ------------------------------------------------------------
\subsection{Неявна частина і роль CPHF}
% ------------------------------------------------------------
...
% ------------------------------------------------------------
\subsection{Підсумковий вираз ядерного гесіана HF}
% ------------------------------------------------------------
