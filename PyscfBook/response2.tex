...
\begin{document}
%% ============================================================
\chapter{Вступ: навіщо потрібні аналітичні похідні}
%% ============================================================
...
g^{(y)}_{ai}
=
\langle \phi_a \,|\, \tfrac{\partial\hat h}{\partial y} \,|\, \phi_i \rangle
+
\langle \phi_a \,|\, \tfrac{\partial\hat V_{\mathrm{HF}}}{\partial y} \,|\, \phi_i \rangle
+ \text{(Pulay)}.
\]
...
=
\hat h_0 - \sum_{\lambda} \mathcal E_\lambda\,\hat\mu_\lambda,
\]
тому
\[
...
F^{(\nu)}_{pq} = -\mu^{(\nu)}_{pq},
\]
змішана друга похідна енергії у HF набуває простого вигляду:
...
Підставляючи це у означення \eqref{eq:alpha_def}, отримуємо остаточний результат:
\begin{equation}\label{eq:HF_polarizability_final}
\alpha_{\mu\nu}
=
2\sum_{ai}
U^{(\mu)}_{ai}\,\mu^{(\nu)}_{ai}.
...
% ---------------------------
% Повний LaTeX-фрагмент з роз'ясненнями і кодовими блоками (minted)
% Вставляти у документ, який підтримує пакет minted.
% ---------------------------

\subsection*{Детальний розбір коду CPHF-процедури (PySCF)}

Нижче подано послідовність кроків коду із стислим математичним описом під кожним блоком та відповідним шматком Python-коду.

\paragraph{1. Параметри і SCF-розрахунок.}
Запускаємо RHF для молекули, отримуємо енергію, коефіцієнти МО, енергії МО та заповнення.

\[
\text{SCF:}\quad \mathbf F \mathbf C = \mathbf S \mathbf C \mathbf \varepsilon,\qquad
\mathbf C = \{\;C_{p i}\;\},
\]
де \(p\) — індекс АО, \(i\) — індекс МО.
...
mo_energy = mf.mo_energy    # ε_i
mo_occ = mf.mo_occ          # заповнення
nocc = np.sum(mo_occ > 0).astype(int)
nvir = len(mo_energy) - nocc
\end{minted}

\paragraph{2. Підготовка: розмір задачі і дипольні інтеграли в АО.}
Дипольні інтеграли в АО задаються \(\mu^{(\nu)}_{pq}\) (компонента \(\nu\in\{x,y,z\}\)). У коді їх отримують викликом інтегратора.
...
\paragraph{3. Перетворення дипольних інтегралів в МО-базис.}
Математично:
\[
\bigl(\mu^{(\nu)}\bigr)^{\mathrm{MO}}_{ij}
= \sum_{p,q} C_{p i}\; \bigl(\mu^{(\nu)}\bigr)^{\mathrm{AO}}_{p q}\; C_{q j}
= \bigl(C^{T}\,\mu^{(\nu),\mathrm{AO}}\,C\bigr)_{ij}.
\]

\begin{minted}{python}
# Перетворюємо в МО базис: μ^MO = C^T · μ^AO · C
dip_ints_mo = np.einsum('pi,xpq,qj->xij', mo_coeff, dip_ints_ao, mo_coeff)
# dip_ints_mo.shape == (3, nmo, nmo)
\end{minted}

\paragraph{4. Двоелектронні інтеграли в AO і проекція на потрібні блоки в МО.}
Повна AO-ERI позначається \((pq|rs)\). Для CPHF потрібні блоки \((ia|jb)\), \((ij|ab)\), \((ib|ja)\), де \(i,j\) — зайняті (occupied), \(a,b\) — віртуальні (virtual).
Загальна формула проекції:
\[
(ia|jb)=\sum_{p q r s} C_{p i}\,C_{q a}\,C_{r j}\,C_{s b}\,(p q|r s).
\]

\begin{minted}{python}
# Двоелектронні інтеграли (AO)
print("Обчислення двоелектронних інтегралів...")
eri_ao = mol.intor('int2e')  # форма (nao, nao, nao, nao)

# Розділяємо матрицю MO на зайняті і віртуальні блоки
C_occ = mo_coeff[:, :nocc]   # форма (nao, nocc)
C_vir = mo_coeff[:, nocc:]   # форма (nao, nvir)
\end{minted}

\paragraph{4a. Побудова блоку \((ia|jb)\).}
Один із способів поступово проектувати AO-ERI у MO-ERI — послідовні contraction-и через \inlinecode{np.einsum}. Код:

\begin{minted}{python}
eri_iajb = np.einsum('pi,pqrs->iqrs', C_occ, eri_ao)   # p -> i
eri_iajb = np.einsum('qa,iqrs->iars', C_vir, eri_iajb)  # q -> a
eri_iajb = np.einsum('rj,iars->iajs', C_occ, eri_iajb)  # r -> j
eri_iajb = np.einsum('sb,iajs->iajb', C_vir , eri_iajb) # s -> b
# Форма: (nocc, nvir, nocc, nvir) = (i, a, j, b)
\end{minted}

\paragraph{4b. Побудова блоку \((ij|ab)\).}

\[
(ij|ab)=\sum_{p q r s} C_{p i}\,C_{q j}\,C_{r a}\,C_{s b}\,(p q|r s)
\]
...
\paragraph{4c. Побудова блоку \((ib|ja)\).}

\[
(ib|ja)=\sum_{p q r s} C_{p i}\,C_{q b}\,C_{r j}\,C_{s a}\,(p q|r s)
\]
...
\paragraph{Примітка.} Послідовні \texttt{einsum} скорочують пам’ять у порівнянні з одноразовою оцінкою повного \((pq|rs)\) у МО-базисі, але все одно масштаб \(O(n^4)\) по числу МО зберігається.

\paragraph{5. Побудова матриці \(H\) (електронного гессіана в просторі параметрів \(U_{ai}\)).}
У моделі HF елемент матриці має вигляд
\[
H_{ai,bj}
=
...
Реалізація (розгортання двох індексів у один): індекс \(k\equiv i\cdot n_\text{vir}+a\).
...
\paragraph{6. Розв'язування CPHF для кожної компоненти зовнішнього поля.}
CPHF у цій реалізації зводиться до прямого розв'язку лінійної системи
\[
H\,U^{(x)} = -g^{(x)},\qquad g^{(x)}_{ia} = -\mu^{(x)}_{ia},
\]
де \(g\) — праві частини, обчислені як елементи дипольної матриці у блоці occupied–virtual.

\begin{minted}{python}
# Розв'язуємо CPHF рівняння для кожної компоненти поля
U = np.zeros((3, nocc, nvir))  # U[x, i, a]
...
\paragraph{Коментар щодо методу розв'язання.}
Тут використано пряму лінійну алгебру (\inlinecode{np.linalg.solve}) — тобто жодної ітераційної процедури в коді немає: матриця \(H\) розв'язується одразу (алгоритмічно --- LU-розклад або гасіння, як реалізовано в NumPy).

\paragraph{7. Обчислення поляризованості.}
Для замкнуто-оболочкової (\texttt{RHF}) системи поляризованість дорівнює

\[
\alpha_{\mu\nu}
=
-2 \sum_{i a} \bigl(\mu^{(\mu)}_{i a} U^{(\nu)}_{i a} + \mu^{(\nu)}_{i a} U^{(\mu)}_{i a}\bigr),
\]
де множник 2 відповідає подвійному заповненню орбіталей.
...
\begin{commentbox}[Короткі додаткові зауваження і оптимізації]
\begin{itemize}
