...
Більшість молекулярних властивостей обчислюється через \textit{теорію відгуку (response theory)}. Ідея проста: прикладаємо зовнішнє збурення до системи
та дивимося, як змінюється хвильова функція або енергія.

\subsubsection*{Розклад енергії в зовнішньому полі}

Для електричного поля $\mathbf{E}$:
...
де:
\begin{itemize}
    \item $E_0$ --- енергія без поля.
    \item $\boldsymbol{\mu}$ --- дипольний момент (перша похідна).
    \item $\alpha_{ij}$ --- поляризовність (друга похідна).
    \item $\beta_{ijk}$ --- перша гіперполяризовність (третя похідна).
\end{itemize}

\textbf{Загальний принцип:} властивість = похідна енергії по зовнішньому полю.
...
\begin{itemize}
    \item Кожна атомна орбіталь має свій локальний gauge
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Анізотропних властивостей
    \end{itemize}
...
\begin{itemize}
    \item H₂O --- прототип для H-зв'язків
    \item Бензен --- ароматичність
    \item O₂, NO --- парамагнетики
...
\begin{itemize}
    \item[$\square$] Геометрія оптимізована? (немає уявних частот)
    \item[$\square$] Базис підходить для цієї властивості?
    \item[$\square$] Метод підходить? (HF завищує поляризовність, потрібен DFT/MP2)
    \item[$\square$] Для магнітних: gauge-independent метод?
    \item[$\square$] SCF збігся? (check \texttt{mf.converged})
    \item[$\square$] Для радикалів: перевірили $\langle S^2 \rangle$?
\end{itemize}
...
\begin{tcolorbox}[colback=blue!5!white,colframe=blue!75!black,title=Готові до роботи?]
Якщо ви:
\begin{itemize}
    \item Встановили \texttt{pyscf-properties}
    \item Розумієте базову структуру розрахунків у PySCF
    \item Знаєте різницю між HF та DFT
    \item Розумієте, що таке базисний набір
\end{itemize}

\textbf{То ви готові обчислювати молекулярні властивості!}

Починаємо з найпростішого: коливальних спектрів.
\end{tcolorbox}
