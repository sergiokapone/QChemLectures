...
def ccsd_helium_detailed(basis='cc-pvqz'):
    ...
    print(f'CCSD внесок:     {e_ccsd_corr*1000:.6f} mHa '
          f'({abs(e_ccsd_corr/total_corr)*100:.2f}%)')
    print(f'(T) внесок:      {e_t*1000:.6f} mHa '
          f'({abs(e_t/total_corr)*100:.2f}%)')
...
def method_hierarchy_comparison(symbol, spin, basis='cc-pvtz'):
    ...
    # Результати
    print('\n' + '='*80)
    print(f'{"Метод":12s} {"Енергія, Ha":18s} {"Відносно HF, mHa":20s} '
          f'{"% FCI кореляції":18s}')
    print('-'*80)
    ...
    corr_energies = [(methods[m] - e_hf) * 1000
                     for m in method_names[1:]]

    ax2.bar(range(len(corr_energies)), corr_energies,
...
def t1_diagnostic_survey(atoms_list, basis='cc-pvdz'):
    ...
    for symbol, spin in atoms_list:
        ...
        try:
            ...
            print(f'{symbol:6s} {spin:4d} {"ПОМИЛКА":12s} '
                  f'{"Не конвергувало":30s}')
...
\begin{table}[h]
\centering
\caption{Вплив електронної кореляції на різні властивості}
\label{tab:correlation_importance}
\small
\begin{tabular}{lll}
\hline
