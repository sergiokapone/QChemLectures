...
\begin{frame}{Теорія збурень}{}
	...
	\begin{equation*}\label{PerturbedValues}
		E_n = E_n^{(0)} + \opbracket{\psi_n^{(0)}}{\hat{V}}{\psi_n^{(0)}}, \quad
...
\begin{frame}{Теорія збурень}{Парагелій}
	...
	\only<1>{
		...
		\begin{equation*}
			E^{(1)} = \opbracket{\Psi_0}{\hat{V}}{\Psi_0} \equiv J = \frac{Z^6}{\pi^2} \int\limits_{V_1}\int\limits_{V_2}\frac{1}{r_{12}} e^{-2Z(r_1 + r_2)} dV_1dV_2 = + \frac58 Z.
		\end{equation*}
...
\begin{frame}{Варіаційний метод}{}
	...
	\only<1>{%
		...
		\begin{equation*}
			E[\Psi]  = \opbracket{\Psi}{\hat{H}}{\Psi} = \int \Psi^* \hat{H} \Psi d\xi, \quad \int \Psi^*  \Psi d\xi = 1.
		\end{equation*}
...
\begin{frame}{Метод самоузгодженого поля}{Метод Хартрі (1927)}
	...
	\begin{overprint}
		...
		\end{align*}
		\onslide<3>
		Енергія атома методом Хартрі $E = \opbracket{\Psi}{\hat{H}}{\Psi}$:
		\begin{multline*}
