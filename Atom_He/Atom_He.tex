...
%============================================================================
\begin{frame}{Eнергетичні рівні атома He}{}
	%----------------------------------------------------------------------------
	\begin{columns}
...
\begin{frame}{Теорія збурень}{}
	...
	\begin{equation*}\label{PerturbedValues}
		E_n = E_n^{(0)} + \opbracket{\phi_n^{(0)}}{\hat{V}}{\phi_n^{(0)}}, \quad
		\phi_n = \phi_n^{(0)} + \sum\limits_{\substack{m\\m \neq n}} \frac{\opbracket{\phi_n^{(0)}}{\hat{V}}{\phi_m^{(0)}}}{E_n^{(0)} - E_m^{(0)}} \phi_m^{(0)}.
	\end{equation*}
	%----------------------------------------------------------------------------
\end{frame}
...
\begin{frame}{Теорія збурень}{Парагелій}
	...
	\only<1>{
		...
		\begin{equation*}
			E^{(1)} = \opbracket{\Phi_0}{\hat{V}}{\Phi_0} \equiv J = \frac{Z^6}{\pi^2} \int\limits_{V_1}\int\limits_{V_2}\frac{1}{r_{12}} e^{-2Z(r_1 + r_2)} dV_1dV_2 = + \frac58 Z.
		\end{equation*}
...
\begin{frame}{Теорія збурень}{Ортогелій}
	...
	\only<1>{
		...
		Перша поправка до енергії в теорії збурень:
		\begin{multline*}
			E^{(1)} = \opbracket{\Phi_0}{\hat{V}}{\Phi_0} = \\ = \opbracket{(1s)_1(2s)_2}{\hat{V}}{(1s)_1(2s)_2} - \opbracket{(1s)_1(2s)_2}{\hat{V}}{(2s)_1(1s)_2} = J - K.
		\end{multline*}
...
\begin{frame}{Варіаційний метод}{}
	...
	\only<1>{%
		...
		\begin{equation*}
			E[\Phi]  = \opbracket{\Phi}{\hat{H}}{\Phi} = \int \Phi^* \hat{H} \Phi d\xi, \quad \int \Phi^*  \Phi d\xi = 1.
		\end{equation*}
...
\begin{frame}{Варіаційний метод}{Атом гелію}
	...
	\begin{equation*}\label{coordinate_ground_state_parahelium}
		\Phi_0(\vec{r}_1, \vec{r}_2) = (1s)_1(1s)_2 = \frac{\zeta^{3}}{\sqrt\pi}e^{-\zeta(r_1 + r_2)}.
	\end{equation*}
...
\begin{frame}{Метод самоузгодженого поля}{Метод Хартрі (1927)}
	...
	\begin{overprint}
		...
		\end{align*}
		\onslide<3>
		Енергія атома методом Хартрі $E = \opbracket{\Phi}{\hat{H}}{\Phi}$:
		\begin{multline*}
			E = 2 \int \phi^*_1 \left( -\frac12 \nabla_1^2 - \frac{1}{r_{1}}\right) \phi_1 dV_1 +  \int\frac{|\phi_1|^2 |\phi_2|^2}{r_{12}} dV_1 dV_2 = \\
...
\begin{frame}{Метод самоузгодженого поля}{Метод Хартрі-Фока}
	...
	\begin{overprint}
		...
		\onslide<4>
		Енергія атома методом Хартрі-Фока $E = \opbracket{\Phi}{\hat{H}}{\Phi}$:
		\begin{multline*}
