...
\begin{frame}{Стаціонарне рівняння Шредінґера атома \ce{He}}{}
	%----------------------------------------------------------------------------
	\begin{equation*}
		\tcbhighmath{\hat{H}\Phi(\vxi_1, \vxi_2) = E\Phi(\vxi_1, \vxi_2).}
	\end{equation*}
	\begin{columns}[T]
...
\begin{frame}{Теорія збурень}{}
	...
	\begin{equation*}
		\hat{H}^0\phi_n^{(0)} = E_n\phi_n^{(0)}.
	\end{equation*}
	Функції $\{\phi_n^{(0)}\}$ --- орбіталі.% --- метод збурень природнім способом реалізує одноелектронне наближення.

	Доданок $\hat{V}$ в припущенні <<малості>> --- <<збурення>>.

	Перше наближення теорії збурень:
	\begin{equation*}\label{PerturbedValues}
		E_n = E_n^{(0)} + \bracket<\phi_n^{(0)}|\hat{V}|\phi_n^{(0)}>, \quad
		\phi_n = \phi_n^{(0)} + \sum\limits_{\substack{m\\m \neq n}} \frac{\bracket<\phi_n^{(0)}|\hat{V}|\phi_m^{(0)}>}{E_n^{(0)} - E_m^{(0)}} \phi_m^{(0)}.
	\end{equation*}
	%---------------------------------------------------------------------------
\end{frame}
...
\begin{frame}{Теорія збурень}{Парагелій}
	...
	\only<1>{
		...
		\begin{equation*}
			\Phi_0(\vec{r}_1, \vec{r}_2) = (1s)_1(1s)_2, \quad (1s) = \frac{Z^{3/2}}{\sqrt{4\pi}} e^{-Zr} .
		\end{equation*}
		...
		\begin{equation*}
			E^{(1)} = \bracket<{\Phi_0}|{\hat{V}}|{\Phi_0}> \equiv J = \frac{Z^6}{\pi^2} \int\limits_{V_1}\int\limits_{V_2}\frac{1}{r_{12}} e^{-2Z(r_1 + r_2)} dV_1dV_2 = + \frac58 Z.
		\end{equation*}
...
\begin{frame}{Теорія збурень}{Ортогелій}
	...
	\only<1>{
		...
		\begin{equation*}
			\Phi_0(\vec{r}_1, \vec{r}_2) = \frac1{\sqrt{2}}[(1s)_1(2s)_2 -(2s)_1(1s)_2],
		\end{equation*}
		\[
		...
		Перша поправка до енергії в теорії збурень:
		\begin{multline*}
			E^{(1)} = \bracket<\Phi_0|\hat{V}|\Phi_0> = \\ = \bracket<(1s)_1(2s)_2|\hat{V}|(1s)_1(2s)_2> - \bracket<(1s)_1(2s)_2|\hat{V}|(2s)_1(1s)_2> = J - K.
		\end{multline*}
...
\begin{frame}{Варіаційний метод}{}
	%----------------------------------------------------------------------------
	\only<1>{%
		Енергію системи в стані $\Phi$ можна розрахувати як:
		\begin{equation*}
			E[\Phi]  = \bracket<{\Phi}|{\hat{H}}|{\Phi}> = \int \Phi^* \hat{H} \Phi d\xi, \quad \int \Phi^*  \Phi d\xi = 1.
		\end{equation*}
	...
	\only<2>{%
		...
		\begin{equation*}
			E[\Phi]  = \bracket<{\tilde\Phi}|{\hat{H}}|{\tilde\Phi}> = \sum\limits_m |C_m|^2 E_m.
		\end{equation*}

		Нехай $E_0$ найменше  значення енергії основного стану гамільтоніана $\hat{H}$, тоді
		\begin{equation*}
			E[\tilde\Phi]  = \bracket<{\tilde\Phi}|{\hat{H}}|{\tilde\Phi}> = \sum\limits_m |C_m|^2 E_m \ge \sum\limits_m |C_m|^2 E_0  = E_0.
		\end{equation*}
	...
	{    \framesubtitle<3>{Реалізація методу}
		Варіаційний метод полягає в тому, щоб використати для розв'язку якусь пробну функцію змінних системи  $\Phi (\lambda_i)$, що залежить від декількох параметрів $\lambda_i$, яка задовільняє умові нормування, тоді:
		\[
			E[\tilde\Phi]  = E[\tilde\Phi(\lambda_1, \lambda_2, \ldots)],
		\]
		умова мінімуму дає $\delta E[\Phi] = 0$:
		\[
		...
		\[
			E_{\min}[\tilde\Phi(\lambda_{1_{\min}}, \lambda_{2_{\min}}, \ldots)] \ge E_0.
		\]
	}
	%----------------------------------------------------------------------------
\end{frame}
...
\begin{frame}{Варіаційний метод}{Атом гелію}
	...
	\begin{equation*}\label{coordinate_ground_state_parahelium}
		\Phi_0(\vec{r}_1, \vec{r}_2) = (1s)_1(1s)_2 = \frac{\zeta^{3}}{\pi}e^{-\zeta(r_1 + r_2)}.
	\end{equation*}
...
\begin{frame}{Метод самоузгодженого поля}{Метод Хартрі (1927)}
	...
	\begin{overprint}
		...
		\begin{itemize}\small
			\item Кожен електрон характеризується своєю хвильовою функцією: $\phi_1$ та $\phi_2$, відповідно, які нормовані $\int |\phi_1|^2 dV_1 = 1$,  $\int |\phi_2|^2 dV_2 = 1$;
			\item Хвильова функція системи електронів задається у вигляді: $\Phi = \phi_1 \phi_2$;
			\item Рівняння методу виводяться на основі варіаційного принципу.
		\end{itemize}
		\onslide<2>
		Рівняння Хартрі (в атомній системі одиниць):
		\begin{align*}
			\left(-\frac12 \nabla_1^2  - \frac{1}{r_1} \right)\phi_1 + \int\frac{|\phi_2|^2}{r_{12}} dV_2 \cdot \phi_1 & = \varepsilon_1 \phi_1, \\
			\left(-\frac12 \nabla_2^2  - \frac{1}{r_2}\right)\phi_2 + \int\frac{|\phi_1|^2}{r_{12}} dV_1 \cdot \phi_2  & = \varepsilon_2 \phi_2.
		\end{align*}
		\onslide<3>
		Енергія атома методом Хартрі $E = \bracket<{\Phi}|{\hat{H}}|{\Phi}>$:
		\begin{multline*}
			E =  \int \phi^*_1 \left( -\frac12 \nabla_1^2 - \frac{1}{r_{1}}\right) \phi_1 dV_1 + \int \phi^*_2 \left( -\frac12 \nabla_2и и ^2 - \frac{1}{r_{2}}\right) \phi_2 dV_2 + \\ +  \int\frac{|\phi_1|^2 |\phi_2|^2}{r_{12}} dV_1 dV_2
			= \varepsilon_1 + \varepsilon_2 - \int\frac{|\phi_1|^2 |\phi_2|^2}{r_{12}} dV_1 dV_2.
		\end{multline*}
	\end{overprint}
	%----------------------------------------------------------------------------
\end{frame}
	...
	\begin{columns}[T]
		\begin{column}{0.35\linewidth}
			\begin{tikzpicture}[
				...
				\node (node0) [io] {$\{\phi_1^{(0)}, \phi_2^{(0)}\}$};
				\node (node1) [process, below of=node0]  {$\int \frac{|\phi_1^{(k)}|^2}{r_{12}} dV_1$, $\int \frac{|\phi_2^{(k)}|^2}{r_{12}} dV_2$};
				\node (node2) [process, below of=node1]               {Розв'язок рівняннь Хартрі};
				\node (node3) [io, below=0.5cm of node2]               {$\{\phi_1^{(k)}, \phi_2^{(k)}\}$};
		...
		\begin{column}{0.55\linewidth}\footnotesize
			...
			\begin{itemize}
				\item  На першому етапі необхідно задати набір деяких початкових функцій $\{\phi_1^{(0)}, \phi_2^{(0)}\}$ в \alert{чисельному вигляді}. Можна вибрати функції можна обрати атомні орбіталі воднеподібного атома.
...
\begin{frame}{Недоліки методу Хартрі}{}
	%----------------------------------------------------------------------------
	Оскільки, для отримання $\Phi$-функції системи необхідно перемножити орбіталі, а тому:
	\begin{itemize}
...
\begin{frame}{Метод самоузгодженого поля}{Метод Хартрі-Фока}
	...
	\begin{equation*}
		...
		{
		\begin{matrix}
			\vphi_{1}(\vxi_1) & \vphi_{2}(\vxi_1) \\
			\vphi_{1}(\vxi_2) & \vphi_{2}(\vxi_2)
		\end{matrix}
		}
		\right|.
	\end{equation*}
	...
	\begin{overprint}
		...
		Для реалізації метода необхідні припущення:
		\begin{itemize}
			\item Кожен електрон характеризується своєю хвильовою функцією: $\vphi_1$ та $\vphi_2$, відповідно, які нормовані $\int |\vphi_1|^2 dV_1 = 1$,  $\int |\vphi_2|^2 dV_2 = 1$;
			\item Рівняння методу виводяться на основі варіаційного принципу.
		\end{itemize}
		\onslide<2>
		...
		{\footnotesize
		\begin{align*}
			\hat{h}_1\vphi_1(\vxi_1) +
			\int\frac{|\vphi_2(\vxi_2)|^2}{r_{12}} d(2) \cdot \vphi_1 (\vxi_1) -
			\int \frac{\vphi_2(\vxi_2)\vphi_1(\vxi_2)}{r_{12}}d(2)\cdot\vphi_1(\vxi_1) \\
			 & = \varepsilon_1 \vphi_1(\vxi_1),                                        \\
			% ----------------
			\hat{h}_2\vphi_2(\vxi_2) +
			\int\frac{|\vphi_1(\vxi_1)|^2}{r_{12}} d(1) \cdot \vphi_2 (\vxi_2) -
			\int \frac{\vphi_1(\vxi_1)\vphi_2(\vxi_1)}{r_{12}}d(1)\cdot\vphi_2(\vxi_2) \\
			 & = \varepsilon_2 \vphi_2(\vxi_2).
		\end{align*}}
		\onslide<3>
		Канонічні рівняння Хартрі-Фока (в атомній системі одиниць):
		\begin{align*}
			\hat{F}_1\vphi_1(\vxi_1) & = \varepsilon_1 \vphi_1(\vxi_1), \\
			% ----------------
			\hat{F}_2\vphi_1(\vxi_2) & = \varepsilon_2 \vphi_1(\vxi_2),
		\end{align*}
        ...
        \end{block}
		\onslide<4>
		Енергія атома методом Хартрі-Фока $E = \bracket<{\Phi}|{\hat{H}}|{\Phi}>$:
		\begin{multline*}
			E = 2 \int \vphi^*_1 \left( -\frac12 \nabla_1^2 - \frac{1}{r_{1}}\right) \vphi_1 d(1) +  \int\frac{|\vphi_1(1)|^2 |\phi_2(2)|^2}{r_{12}} d(1) d(2) - \\
			- \int\frac{\vphi_1 (1) \vphi_2(2) \vphi_2 (1) \vphi_1(2)}{r_{12}} d(1) d(2)
			= \\
			= \varepsilon_1 + \varepsilon_2 - \left(J_{12} - K_{12}\right) .
		\end{multline*}
		\onslide<5>
		\(%
		J_{12} = \int\dfrac{|\vphi_1(1)|^2 |\phi_2(2)|^2}{r_{12}} d(1) d(2)%
		...
		\(%
		K_{12} = \int\dfrac{\vphi_1 (1) \vphi_2(2) \vphi_2 (1) \vphi_1(2)}{r_{12}} d(1) d(2)%
		\) -- обмінний інтеграл.
...
\begin{frame}{Метод Хартрі-Фока}
	...
	\only<1>{
		...
		У 1951 році Рутаан запропонував представляти орбіталі Хартрі-Фока у вигляді лінійної комбінації $\phi = \sum\limits_{s = 1}^\infty c_s \chi_s$ повного набору відомих функцій $\chi_s$, які називаються \alert{базисними функціями}.

		\medskip}

	На практиці, зазвичай, обирають лише $M$ базисних фукнцій $\chi_i$:
	\begin{equation*}\label{}
		\phi = \sum\limits_{s = 1}^M c_s \chi_s.
	\end{equation*}
...
\begin{frame}{Метод Хартрі-Фока}{Атом гелію}
	...
	1s-Орбітальну функцію атома гелію $\phi$ можна представити як комбінацію двох 1s-орбіталей ($n = 1$) слейтерівського типу:
	\begin{equation*}\label{}
		\phi = \pi^{-1/2}\sum\limits_{s = 1}^2 c_s \zeta_s^{3/2}e^{-\zeta_s r},
	\end{equation*}
		...
		{\footnotesize\color{blue}\ttfamily
				\begin{NiceTabular}{llc}
					...
					\begin{tikzpicture}[>=stealth, baseline=-2cm]
						\node (1s)  [fit=(1-3)(1-3)] {} ;
						\draw[<-] (1s.north) -- ++(0,0.5) node[above] (phi) {$\phi_{1s}$};
...
\begin{frame}[fragile]{Уточнення розрахунків}
	...
	\begin{onlyenv}<1>
		...
		\begin{equation*}
			\phi = c_1 \chi(\zeta_1) + c_2 \chi(\zeta_2),
		\end{equation*}
...
\end{verbatim}
		...
		\vspace*{-1cm}
		\[
			\phi_{0} = 0.592081 \cdot \sum_{i = 1}^3 C_i GTO(\alpha_i)  + 0.513586 \cdot C_1  GTO(\alpha_1),
		\]
		\begin{alertblock}\scriptsize
			В результатах розрахунку з'являється ще одна орбіталь $\phi_1$ з коефіцієнтами $-1.149818$ та $1.186959$ і орбітальною енергією $1.39986$, яка не заселена електронами і є артефактом вибору базису. Така орбіталь називається \alert{віртуальною} і фізичного трактування при атомних розрахунках не має.
		\end{alertblock}
	\end{onlyenv}
