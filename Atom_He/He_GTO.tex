...
Базиси для представлення орбіталей:

\begin{equation*}
    \phi(x, y, z) = \sum_{i=1}^M c_i \chi_i(x, y, z | \zeta).
\end{equation*}

\section{Орбіталі слейтерового типу (STO) та орбіталі гаусового типу}

\href{https://en.wikipedia.org/wiki/Gaussian_orbital}{Орбіталі слейтерового типу (STO)}:
...
\href{https://en.wikipedia.org/wiki/Slater-type_orbital}{Орбіталі гаусового типу (GTOs)}:
...
Коли $i + j + k = 0$ (такі що, $i = 0$, $j = 0$, $k = 0$), GTO називається $s$-типу.

Якщо $i + j + k = 1$, маємо гауссову функцію $p$-типу, яка містить множник $x$,
$y$, or $z$. Якщо $i + j + k = 2$, ми маємо $d$-тип GTO.

Різниця між STO і GTO полягає у $r$-експоненті. GTO підносить $r$ до квадрату, так що добуток гауссових <<примітивів>> (вихідних гауссових рівнянь) є іншою гауссовою величиною. Таким чином, ми отримуємо рівняння, з яким можна працювати, і яке значно спрощується. Однак, ціна, яку ми платимо --- це втрата точності. Щоб компенсувати цю втрату, ми виявили, що чим більше гауссових рівнянь ми комбінуємо, тим точніше наше рівняння.
...
\begin{figure}[!h]\centering
	\begin{tikzpicture}[
		...
		\end{axis}
	\end{tikzpicture}
\caption{Представлення STO трьома GTO}
\end{figure}

%% --------------------------------------------------------
\clearpage
\section{Побудова STO на основі базису}
%% --------------------------------------------------------
...
\begin{multline*}
	\mathrm{STO}_1 = \sum_{i = 1}^3 C_i \cdot GTO(\alpha_i) = \\
...
\begin{multline*}
	\mathrm{STO}_2 =  C_i \cdot GTO(\alpha_i) = \\ = \C{1.0000000000}\cdot\sgto{0.2975781595}.
\end{multline*}
...
\begin{align*}
	\phi_{0} & = \C{0.592081} \cdot \mathrm{STO}_1  + \C{0.513586} \cdot \mathrm{STO}_1, \quad \text{двічі заселена орбіталь}           \\
	\phi_{1} & = \C{-1.149818} \cdot \mathrm{STO}_1  + \C{1.186959} \cdot \mathrm{STO}_1. \quad \text{віртуальна (незаселена) орбіталь}
\end{align*}
