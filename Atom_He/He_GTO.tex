...
\emph{Базисний набір} --- сукупність функцій, що слугують для представлення наближених орбіталей атома (чи молекули).  Чим точніше цей набір обрано, тим точніше можна отримати результати при розв'язанні рівнянь Хартрі-Фока, що визначають електронну структуру системи.

Розкладання орбіталі $\phi$ через \emph{базисні} функції (базисні орбіталі) $\chi_i$:
...
\begin{itemize}
	...
	\item \href{https://en.wikipedia.org/wiki/Gaussian_orbital}{Орбіталі гаусового типу (GTO)}:
...
У GTO підносить $r$ підноситься до квадрату, так що добуток гауссових функцій є іншою гауссовою функцією. Цим самим, ми отримуємо інтеграли, з яким легше працювати, а також прискорюємо їх обрахунки. Однак, ціна, яку ми платимо --- це втрата точності. Щоб компенсувати цю втрату, виявлено, що чим більше гауссових рівнянь ми комбінуємо, тим точніше результат.
...
Базис \href{https://www.basissetexchange.org/basis/sto-3g/format/orca/?version=1&elements=2}{STO-3G} з \cite{BSE}:
...
Базис можна зобразити на двовимірному графіку (рис.~\ref{pig:STO-3G}). На рис.~\ref{pig:STO-3G} також наведена <<справжня>> STO (штрихованою лінією). Ця орбіталь була побудована за формулою \eqref{STO}, при $\zeta = 1.7$ (екранований заряд). Як видно з графіку, STO-3G добре апроксимує STO за винятком області біля ядра.
...
Базис \href{https://www.basissetexchange.org/basis/6-31g/format/orca/?version=1&elements=2}{6-31G} з \cite{BSE}:
...
\begin{align*}
	\phi_{0} & = \C{0.592081} \cdot \mathrm{STO}_{1s}  + \C{0.513586} \cdot \mathrm{STO}_{2s},           \\
	\phi_{1} & = \C{-1.149818} \cdot \mathrm{STO}_{1s}  + \C{1.186959} \cdot \mathrm{STO}_{2s}.
\end{align*}
\end{tcolorbox}

Орбіталь $\phi_0$ --- двічі заселена, орбіталь $\phi_1$ --- не заселена електронами. Не заселені орбіталі називаються \emph{віртуальними}.
...
Віртуальні орбіталі не мають фізичного сенсу (артефакт методу)\footnote{Однак вони використовуюся в деяких пост-Хартрі-Фоківських методах.}.
