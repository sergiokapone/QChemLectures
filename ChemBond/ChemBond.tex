...
\begin{frame}{Природа хімічного зв'язку}
	...
	\only<1>{
		Для пояснення природи хімічного зв'язку історично виникло два методи:
		\begin{enumerate}
	...
	\only<2->{
		...
		\begin{enumerate}
			\item Кожному електрону в молекулі відповідає $\phi$-функція~--- \alert{молекулярна орбіталь} (МО).
			      \begin{alertblock}{}
				      Електрон делаокалізований і належить не конкретному атому, а молекулі!
			      \end{alertblock}
			\item Молекулярна орбіталь будується як лінійна комбінація атомних орбіталей (\alert{МО-ЛКАО}).
			...
			\item Технічна реалізація метода МО-ЛКАО --- метод Хартрі-Фока-Рутаана.
		\end{enumerate}
		\begin{center}
			\alert<3>{
				Ідея методу молекулярних орбіталей~--- задача про найпростішу молекулярну систему~--- молекулярний іон водню \ce{H^+_2}
			}
		\end{center}
	}
...
\begin{frame}[t]{Молекулярний іон \ce{H^+_2}}{Нормувальний множник та інтеграл перекриття}
	...
	\begin{multline*}
		...
		= \frac{c^2}{2} \left( 2 \pm 2\int\chi_a^\text{АО}\chi_b^\text{АО} dV \right) = c^2 \left( 1 \pm  S \right) = 1
	\end{multline*}
	\begin{overprint}
		\onslide<1>
...
\begin{frame}{Молекулярний іон \ce{H^+_2}}{Інтеграл перекриття. Фізичний смисл}
	...
	\begin{overprint}
		...
		\begin{tikzpicture}[scale=0.9, >=latex]
			\node[circle, fill=red, inner sep=1pt] (x0) at (0,0)   {};
			\node[circle, fill=red, inner sep=1pt] (x1) at (0.5,0) {};
...
%			\node at (x0) {\textbullet};
			%			\node[below left=25pt of x0] {$\chi_a$};
%			\node at (x1) {\textbullet};
			%			\node[below right=25pt of x1] {$\chi_b$};
			\node[below] at (0.25, -1.1) {$S_{ab} \approx 1$};
			\draw[<->, gray] (x0) -- node[below] {$R_{ab}$}(x1);
			%==========================================================
			\node[circle, fill=red, inner sep=1pt] (y0) at (3.5,0) {};
			\node[circle, fill=red, inner sep=1pt] (y1) at (5,0) {};
...
%			\node at (y0) {\textbullet};
			%			\node[below left=25pt of y0] {$\chi_a$};
%			\node at (y1) {\textbullet};
			%			\node[below right=25pt of y1] {$\chi_b$};
			\node[below] at (4.25, -1.1) {$0 < S_{ab} < 1$};
			\draw[<->, gray] (y0) -- node[below] {$R_{ab}$}(y1);
			%==========================================================
			\node[circle, fill=red, inner sep=1pt] (z0) at (7.5,0) {};
			\node[circle, fill=red, inner sep=1pt] (z1) at (9.75,0) {};
...
%			\node at (z0) {\textbullet};
			%			\node[below left=25pt of z0] {$\chi_a$};
%			\node at (z1) {\textbullet};
			%			\node[below right=25pt of z1] {$\chi_b$};
			\node[below] at (8.625, -1.1) {$S_{ab} \approx 0$};
			\draw[<->, gray] (z0) -- node[below] {$R_{ab}$}(z1);
		\end{tikzpicture}
	\end{overprint}
...
\begin{frame}{Молекулярний іон \ce{H^+_2}}{Енергія системи}
	Енергії системи:
	\begin{equation*}
		E_{S,A} = \bracket<\phi_{S,A}|\hat H|\phi_{S,A}> = \frac{H_{aa} \pm H_{ab}}{1\pm S},
	\end{equation*}
	\begin{overprint}
		\onslide<1>
		Розглянемо інтеграли:
		\begin{align*}
			H_{aa} = H_{bb}  & = \bracket<\chi_{a}|\hat H|\chi_a> = \bracket<\chi_{b}|\hat H|\chi_{b}> \\
			H_{ab}  = H_{ba} & = \bracket<\chi_{a}|\hat H|\chi_{b}> = \bracket<\chi_{b}|\hat H|\chi_{a}>
		\end{align*}
		\onslide<2>
		\begin{equation*}
			H_{aa} = \bracket<\chi_{a}|\hat H|\chi_{a}> = \bracket<\chi_{a}|\hat H_a - \frac{1}{r_b} + \frac1{R_{ab}}|\chi_{a}> 		= E_H + J + \frac{1}{R_{ab}},
		\end{equation*}
		де \(J = \bracket<\chi_{a}|-\frac1{r_b}|\chi_{a}>\) --- кулонівський інтеграл.
		\onslide<3>
		\begin{equation*}
			H_{ab} = \bracket<\chi_{a}|\hat H|\chi_{b}> = \bracket<\chi_{a}|\hat H_a - \frac1{r_b} + \frac1{R_{ab}}|\chi_{b}> =  E_HS + \beta + \frac{1}{R_{ab}}S,
		\end{equation*}
		де \(\beta = \bracket<\chi_a|-\frac1{r_b}|\chi_b> = \bracket<\chi_b|-\frac1{r_a}|\chi_a>\) --- резонансний інтеграл.
		\onslide<4>
		Повна енергія системи дорівнює:
		\begin{equation*}
			E(R_{ab}) = E_H + \frac1{R_{ab}} + \frac{J\pm\beta}{1\pm S}
		\end{equation*}
	\end{overprint}
\end{frame}
...
З формули для $J$ видно, що кулонівська енергія та енергія між'ядерного відштовхування значною мірою компенсують одна одну, отже, зв'язок виникає здебільшого завдяки <<резонансній>> енергії ($\beta$ --- від'ємна величина), тобто внаслідок притягання <<заряду перекривання>> позитивними ядрами остова.
\end{alertblock}
\end{frame}
	...
	\begin{columns}[c]
		...
		\begin{column}{.5\linewidth}
			\begin{overprint}
				...
				\end{equation*}
				\begin{equation*}
					\Delta E = E - E_H = \frac1{R_{ab}} + \frac{J+\beta}{1 + S}
				\end{equation*}
				...
				\end{equation*}
				\begin{equation*}
					\Delta E = E - E_H = \frac{1}{R_{ab}} + \frac{J-\beta}{1- S}
				\end{equation*}
				\onslide<2>
				\begin{equation*}
					\phi_{A} = \frac{1}{\sqrt{2(1-S)}}\left( \chi_{a} -  \chi_{b}\right)
...
\begin{frame}{Енергія молекули}
	...
	Енергія молекули:
	\begin{equation*}
		\tcbhighmath{E_{S,A} = \bracket<\Phi_{S,A}|\hat H|\Phi_{S,A}> = 2E_H^0 + \frac{Q \pm I}{1\pm S^2},}
	\end{equation*}
	\begin{overprint}
		\onslide<1>
		\begin{equation*}
		...
		\begin{equation*}
			J = \bracket<{|\chi_a(1)|^2}|{\frac{1}{r_{ab}}}|{|\chi_b(2)|^2}> = \bracket<{|\chi_a(2)|^2}|{\frac{1}{r_{12}}}|{|\chi_b(1)|^2}>
		\end{equation*}
		\onslide<2>
		\begin{equation*}
		...
		\begin{equation*}
			K = \bracket<{\chi_a(1)\chi_b(2)}|{\frac{1}{r_{12}}}|{\chi_b(1)\chi_a(2)}>.
		\end{equation*}
	\end{overprint}
\end{frame}
%====================================================================================
...
% ============================== Слайд ## ===================================
\begin{frame}{Порівняння методів МО-ЛКАО та ВС}{}
        \begin{alertblock}{}
            Методи МО-ЛКАО та ВС є наближеними.
        \end{alertblock}
\begin{block}{}
    Для того щоб покращити результату методі BC необхідно вийти за рамки опису суто ковалентного зв'язку і врахувати іонну складову.

    \medskip

    Для покращити методу MO необхідно враховувати взаємодію різних МО --- так звана конфігураційна взаємодії:

    \begin{multline*}
    \Phi = (1s_a(1)) + (1s_b(1))(1s_a(2)) + (1s_b(2)) + \\  + \kappa (1s_a(1)) - (1s_b(1))(1s_a(2)) - (1s_b(2)) = \\ =
    N \left( \Phi_\text{ion} + \lambda \Phi_\text{cov}\right),
    \end{multline*}
    де $\lambda  = \frac{1 - \kappa}{1 + \kappa}$.

    \medskip

    Тобто, якщо уточнити ВС, вводячи іонні стани, а МО, враховуючи конфігураційну взаємодію, то результати, отримані
    обома методами мають збігаються.
